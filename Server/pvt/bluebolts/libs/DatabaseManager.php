<?php
/**	DatabaseManager:  A library designed to help deal with widgets.
*	
...
class DatabaseManager{
	...
	*/
	public static function connectAs($args){
		...
		if($preConns[$args->username]) return $preConns[$args->username];
		
		$connection = @new mysqli($args->hostname, $args->username, $args->password, $args->database);
		if($connection->connect_errno) throw new Exception($args->username+" failed to connect: ".$connection->connect_error);
	...
	*/
	function __construct($connection = null, $type = 'custom'){
		if(!$connection) throw new Exception("Failed to create a DatabaseManager.  Try using 'DatabaseManager::connectAs(<type>)' to create an instance.");
	...
	*/
	public function query($queryString, $vars=null){
		if(!$queryString) throw new Exception($this->type." failed a query: "."No Query String Given.");
		...
		if(!$resource) throw new Exception($this->type." failed query(".$queryString."): ".$this->connection->error);
	...
	*/
	public function queryToString($queryString, $vars){
		if(!$queryString) throw new Exception("queryToString failed:  No Query String Given.");
	...
	*/
	public function queryList($queryString){
		...
		if(!$result) throw new Exception($this->type." failed a query: ".$this->connection->error);
